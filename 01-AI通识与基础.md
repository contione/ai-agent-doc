...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/OzwjbeL7koKrlXxkVtIcag8anTb/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/XNoxbf4WzodF0NxKm7ecECsvnvf/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/EuBEb6a5Po2MD5xx5KncNKT2n4e/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/Zdu7bwP1Lo1606xOjn5cZGdNn2g/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/SyRJbeM5VoE2a8xiPFWciapkn7b/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/OzF7bYocWoZpt0xUYEIceMeUnLc/)

你给它输入一些参数，最终它经过计算返回一个结果。因此从某种意义上，你可以**把模型看做是一个函数**。

> [白板/画板内容] (token: PfUhd3BuVok2hJxnDvQcB2oGn4b)

这就类似：y = ax + b，这个函数有两个参数a和b，当a和b确定时，这个函数就能表示一条直线。输入一个x，一定能得到一个结果y

当然，模型这个“函数”要复杂的多，其参数不是两个，而是可能达到千亿规模：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/Xy3abVi7XoNIp3xEknRcnNUanGc/)

因此它表示的不是一条直线，而是表示人类复杂的语言系统。

**模型训练的过程，就是求模型参数的过程，类似于求解函数参数**。已知直线上两个点的坐标，就能求出这条直线对应的a和b的值。

> [白板/画板内容] (token: VoBwdKmQFo4anMx6GPHcpBsen4y)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/NKlkbt1eEo6i4txC6HcctfYtnog/)

此时，我们用E(美国) - E(中国) 得到的新向量，就可以表示为美国与中国的差异。

假如此时我询问LLM在中国有什么食物与美国的汉堡类似，我们就可以这么做：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/JS1TbJ0YCoXRzoxzzSzcjRepnjb/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/YSSNbwcWdotszsx5BajcqVGxnuB/)
...
[www.youtube.com](https://www.youtube.com/watch?v=wjZofJX0v4M&t=1169s)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/IdUyb8IIvodVdHxZxjrcEnJ0n9d/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/NWfcbupwqomka5xsGtXcUFtRni2/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/K3lrbgKysoakhvxuYgacSChfnhc/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/ZsItbncYzovBWox7epFcffmGnUh/)
...
[嵌入网页](https://api-docs.deepseek.com/zh-cn/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/DGkZb2h7QoP1fmxzA1GcFGFxnZs/)

注意：需要在请求头中添加刚刚我们注册时准备的API_KEY：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/BEXkbXBsTotaLnxlLtRcSF31nDd/)
...
[阿里云登录 - 欢迎登录阿里云，安全稳定的云计算服务平台](https://account.aliyun.com/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/GMSUb7IH3oLk6lx6LzMclv8lnDb/)

点击确认开通后，如果未进行实名认证，会提醒账户异常：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/F1sDbKFhbocEVxxw4tvciouYnFe/)

点击去认证，申请个人认证即可，此处略过。

首次开通应该会赠送百万token的使用权，包括DeepSeek-R1模型、qwen模型等等，有效期是3~9个月不等。大家可以在《模型控制台》-> 《模型用量》查看到你的免费额度使用情况：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/ZW2VbTz1coaYMwxuDg6c2DV5nob/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/V6hvbhau9oN9hbxgO6ucygDXnvb/)

在阿里云百炼平台的左侧菜单的最下方，有一个《密钥管理》选项：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/AB9BbqkaEoiGIUx1rqXcCdamntb/)

点击后，进入《密钥管理》页面，点击创建`API-KEY`：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/CzJ6bcgLNotLaVxSSQ1cEK6gn0d/)

选择`创建API-KEY`后，会弹出表单，只有一个选项，勾选后点击确定即可：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/JU6mbEYG8oNZNUxsTeucACBjnlb/)

点击确定，即可生成一个新的`API-KEY`：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/IlJNbYEgjojcbUxM4EmcUnV4nWh/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/ZS4wbC35Qodg1uxBLKBcnLFBnM0/)

即可进入模型广场：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/X03CbJ5nIoiF0oxmfmJcvd1QnGl/)

### 5.2.4 API文档

点击API参考即可进入API文档页面：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/I3ChbDRHNoIBl3xYzVHcjU80nhd/)

### 5.2.5 测试

我们使用Http客户端来调试（**不要忘了设置API_KEY**）：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/D9O0b30nVoNL0Jxhw3XcFIpunhh/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/XezPbqhnNoUJlhx7smqcvkzAnyw/)

下载后双击就会弹出安装界面：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/Rj7Sbas5XoEUFlxBJCBc5Kj1nP6/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/Pnqrbnfr1oKxhqxRZ6qct5f7njh/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/LSjZbQRepohVPPxRGNhcMIzYn7c/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/ItIyb2HmAowwVexIS9ocIRILnTc/)

点击进入deepseek-r1页面，会发现deepseek-r1也有很多版本：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/Y6dhbWb40oR6HWx6f4sczfydnbd/)

这些就是模型的参数大小，越大推理能力就越强，需要的算力也越高。671b版本就是最强的满血版deepseek-r1了。需要注意的是，Ollama提供的DeepSeek是量化压缩版本，对比官网的蒸馏版会更小，对显卡要求更低。对比如下：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/WuKwbQgFyoIQNPxfNhicTMpAnyP/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/EP2jb1h0Sol7HKxeL7HcbSE7n7b/)

复制这个命令，然后打开一个cmd命令行，运行命令即可，然后你就可以跟本地模型聊天了：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/S4dOb3Teqo3NlCxQd0pcmzutnUb/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/T6ZjbSPmtoqgTBxtFbYc1Ngwnyh/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/WdNlb6eR8owaSvxXbdic10dcnwd/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/RRL2byM5IoyKpDxsckYcyn2Jnvd/)

AI的答案是：`每人可以分到4个苹果。`

我们接着问：`如果是分给4个人呢？`，由于AI没有记忆，它不知道我是接着上一题问的，因此不知道要分的是12个苹果，答案就有问题：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/BeRCbLivdok524xZ2nwcsO3Jnph/)
...
[嵌入网页](https://api-docs.deepseek.com/zh-cn/guides/multi_round_chat)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/MxEGbwyoSobY1bxv3khcLqnannd/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/Fqpjb5R6XoWxVAxCJoYcYrQsnQZ/)
...
[www.bilibili.com](https://www.bilibili.com/video/BV1Stwfe1E7s/?spm_id_from=333.1387.search.video_card.click)

### 6.3.2 添加镜像源

默认情况下，uv下载依赖是到国外站点：[https://test.pypi.org/simple](https://test.pypi.org/simple)，速度很慢。推荐大家将下载的镜像源改为国内站点。
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/PrUzbVV6Zo2dKDx0xJOcfnjKn8e/)

为了方便大家学习，我们会使用jupyter，所以需要在项目中引入notebook依赖。

在PyCharm中，左侧有一个Terminal按钮，点击即可打开终端：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/CaJCbilRgoeUzmxNSCKcGRNonFb/)

如图：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/EbNTbpXOLo5r4DxQmJwcbzCznDf/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/HbQtbwq3yotPipxDqficn7Ifn4c/)

起名为`hello`：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/PE78bqWFRoTATtxpZNGcJ9jpn5c/)

如图：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/SAdfbv8qioCuOgxyNyvcJPtJnNg/)

然后在代码框中编写打印HelloWorld的代码，快捷键`SHIFT `+ `ENTER`即可运行：：

![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/PIlEbyZi1oEqJoxLYJAcveJynsb/)
...
![图片](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/v2/cover/UqcKbaMTvoICWBxtLVecDljUn4d/)
